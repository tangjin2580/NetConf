...
|---------|-----------|----------------|---------|---------|
| **Win7-x64** | 3.8 | 5.13 | Windows 7 (含未打 SP1 的早期版本/RTM) / 8.1 / 10 / 11 (64位) | 旧电脑、Win7系统 |
...
> **Universal CRT（api-ms-win-crt-*.dll / ucrtbase.dll）** 以「应用本地」方式一并打进程序目录，
> 客户机**无需安装任何运行库**即可直接运行。该方式同样兼容**未打 SP1 的 Windows 7 早期版本（RTM）**
> （UCRT 不依赖系统全局运行库，随包加载）。以下仅作为极少数老系统的离线兜底参考。
